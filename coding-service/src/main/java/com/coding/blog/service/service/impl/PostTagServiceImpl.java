...
public class PostTagServiceImpl extends ServiceImpl<PostTagMapper, PostTag> implements IPostTagService {
    ...
    public List<PostTag> queryAllPostTag() {
        List<PostTag> postTagsCache = (List<PostTag>) redisTemplateUtil.get(RedisConstants.REDIS_KEY_POST_TAG);
        if (CollUtil.isEmpty(postTagsCache)) {
            List<PostTag> postTags = postTagMapper.selectList(null);
            redisTemplateUtil.set(RedisConstants.REDIS_KEY_POST_TAG, postTags);
            postTagsCache = postTags;
        }
        return postTagsCache;
    }
